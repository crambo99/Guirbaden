...
public partial class Plan : ContentPage
{
    ...
    private void OnPinchUpdated(object sender, PinchGestureUpdatedEventArgs e)
    {
        ...
        switch (e.Status)
        {
            case GestureStatus.Started:
                // Store the current scale factor applied to the wrapped user interface element,
                // and zero the components for the center point of the translate transform.
                startScale = planView.Scale;
                planView.AnchorX = e.ScaleOrigin.X;
                planView.AnchorY = e.ScaleOrigin.Y;
                break;
            case GestureStatus.Running:
                // Calculate the scale factor to be applied.
                currentScale += (e.Scale - 1) * startScale;
                currentScale = Math.Max(1, currentScale);
                planView.Scale = currentScale;
                break;
    ...
    {
        planView.TranslationX += e.TotalX;
        planView.TranslationY += e.TotalY;
    }
}